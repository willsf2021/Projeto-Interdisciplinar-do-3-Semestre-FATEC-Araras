...
export const ListItem = ({ type, item, onClick, onRemove }) => {
  ...
  return (
    <ItemCard onClick={onClick} clickable={!!onClick}>
      <ItemHeader>
        <ItemIcon>{isDocument ? <FileEarmarkText /> : <Person />}</ItemIcon>

        <ItemContent>
          <ItemTitle>
            {isDocument ? item.receita_nome : item.nome_completo || "Sem nome"}
          </ItemTitle>
          ...
          <ItemDescription>
            {isDocument ? (
              <>
                <FileText size={12} />
                {item.receita_categoria || "Documento sem descrição"}
              </>
            ) : (
              <>
                <Envelope size={12} />
